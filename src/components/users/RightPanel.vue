  ...
  <aside class="right-panel">
    ...
    <div class="rp-body">
      <transition name="fade-swap" mode="out-in">
        ...
        <!-- Recommendations -->
        <div v-if="tab === 'rec'" key="rec" class="rp-rec">
          <div v-if="!recommendations.length" class="rp-empty">
            <SparklesIcon class="rp-empty-icon" />
            <p>Play a few tracks to get personalized picks</p>
          </div>
          <div v-for="track in recommendations" :key="track._id" class="rp-item" @click="$emit('select-track', track)">
            ...
            <div class="rp-actions">
              <button class="rp-act-btn" @click.stop="$emit('play-track', track)" title="Play">
                <PlayIcon class="rp-act-icon" />
              </button>
              <button class="rp-act-btn" @click.stop="$emit('add-to-queue', track)" title="Queue">
        ...
        <!-- Queue -->
        <div v-else key="queue" class="rp-queue-tab">
          <div v-if="currentMusic" class="rp-now">
            <span class="rp-now-label">Now playing</span>
            <div class="rp-now-row">
              <img :src="getCover(currentMusic)" class="rp-now-cover" alt="" @error="e => e.target.src = fallback" />
          ...
          <template v-else>
            <div class="rp-queue-head">
              <span class="rp-queue-count">{{ queue.length }} track{{ queue.length !== 1 ? 's' : '' }}</span>
              <button class="rp-clear-btn" @click="$emit('clear-queue')">Clear all</button>
            </div>
            <div v-for="(item, i) in queue" :key="item._id" class="rp-item">
              <span class="rp-q-idx">{{ i + 1 }}</span>
              <img :src="getCover(item)" class="rp-cover" alt="" @error="e => e.target.src = fallback" />
              <div class="rp-info">
...
defineEmits([
  'toggle-queue', 'close-queue', 'play-track',
  'remove-from-queue', 'clear-queue', 'select-track', 'add-to-queue',
])
