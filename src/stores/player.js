...
export const usePlayerStore = defineStore('player', () => {
  ...
  const findQueueIndex = (trackId) =>
    ...
    queue.value.findIndex((item) => String(item?._id || item?.id || '') === String(trackId || ''))
  ...
  const setQueue = (tracks = [], activeTrack = null) => {
    ...
    if (activeTrack?._id || activeTrack?.id) {

      currentTrack.value = activeTrack

      const idx = findQueueIndex(activeTrack._id || activeTrack.id)
      ...
      if (idx >= 0) {
        ...
        queue.value = normalizeQueue([activeTrack, ...nextQueue])
    ...
    currentTrack.value = nextQueue[0]
  ...
  const setTrack = (track, options = {}) => {
    ...
    const idx = findQueueIndex(track._id || track.id)
    ...
    if (idx >= 0) {
      ...
      queue.value = normalizeQueue([track, ...queue.value])
  ...
  const addToQueue = (track) => {
    ...
    if (!track?._id && !track?.id) return false

    const id = String(track._id || track.id)

    const exists = queue.value.some((item) => String(item?._id || item?.id || '') === id)
    ...
    if (shuffle.value && queue.value.length > 1) {
      ...
      const baseIndex =
        ...
        queueIndex.value >= 0
          ...
          : findQueueIndex(currentTrack.value?._id || currentTrack.value?.id)
  ...
  const removeFromQueue = (trackId) => {
    ...
    const removedIndex = prev.findIndex((item) => String(item?._id || item?.id || '') === id)

    const wasCurrent = String(currentTrack.value?._id || currentTrack.value?.id || '') === id

    queue.value = prev.filter((item) => String(item?._id || item?.id || '') !== id)
    ...
    if (wasCurrent) {
      ...
      const fallbackIndex = Math.min(removedIndex, queue.value.length - 1)
    ...
    queueIndex.value = findQueueIndex(currentTrack.value?._id || currentTrack.value?.id)
